...
exports.inferSchemaObject = exports.inferTableObject = exports.inferSchema = exports.inferTable = void 0
...
`
async function inferTable(connectionString, table, prefix, defaultsInterfaces) {
  const db = new mysql_client_1.MySQL(connectionString)
  const code = typescript_1.tableToTS(table, prefix || '', await db.table(table), defaultsInterfaces)
  const fullCode = `
...
async function inferSchema(connectionString, prefix, defaultsInterfaces) {
  const db = new mysql_client_1.MySQL(connectionString)
  const tables = await db.allTables()
  const interfaces = tables.map(table =>
    typescript_1.tableToTS(table.name, prefix || '', table.table, defaultsInterfaces)
  )
